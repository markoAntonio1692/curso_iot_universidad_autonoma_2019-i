...
void loop() {
  ...
  if (now - lastMsg > 1000) {
    ...
    //String var_boton;
    //var_boton=String(boton);
    //Serial.println(var_boton);


   char sense[]=""; //almacenar dato en char array
   
    //variable boton contien estado logico del pin d7
   String my_payload=String(boton); //convierto luego a string
    my_payload.toCharArray(sense, (my_payload.length() + 1)); //luego convierto a char array 
    client.publish("boton", sense); //siempre recibe un char array 
